...
NotificationLibrary:SendNotification("Xin chào cả nhà nhá!")
NotificationLibrary:SendNotification("Rất vui khi bạn sử dụng script <3")
---------
...
local Window = Fluent:CreateWindow({
	Title = "Peguin hub Beta",
	SubTitle = "Xin chào cả nhà!",
	TabWidth = 160,
...
Tabs.infor:AddButton({
	Title = "Discord | Penguin Hub",
	Description = "Copy Link Discord",
	Callback = function()
		setclipboard("discord.gg/PenguinHub")
	end
})
---------------TabStatus
...
local Bluec = Tabs.Home:AddParagraph({
	Title = "Elite Status",
	Content = ""
})
spawn(
...
Tabs.Home:AddButton({
	...
	Callback = function()
		game:GetService("TeleportService"):TeleportToPlaceInstance(game.placeId, IAMGAY, game.Players.LocalPlayer)
	end
})
------//Farm Ming Di Cac Ban Oi

local SelectFastAttackMode = (SelectFastAttackMode or "Supper Fast")
...
local SelectedFastAttackModes = Tabs.Main:AddDropdown({
...
--//
local Bluea = Tabs.Main:AddToggle("Bypass TP", {
	Title = "Bypass TP",
	Default = false
})
Bluea:OnChanged(function(Value)
	BypassTP = Value
end)

local Bluet = Tabs.Main:AddToggle("Bypass TP", {
	Title = "Stop TP Bypass When Have Legendary",
	Default = false
})
Bluet:OnChanged(function(Value)
	_G.StopTP = Value
end)
...
local Bluee = Tabs.Main:AddToggle({
	Title = "Auto Race V4",
	Default = false
})
Bluee:OnChanged(function(Value)
	Racelord = Value
end)
...
local Bluess = Tabs.Main:AddToggle({
	Title = "Auto farm lv",
	Description = ""
})

Bluess:OnChanged(function(Value)
	FarmMode = Value
end)
...
local bonebeaa = Tabs.Main:AddToggle({
	Name = "Auto farm bone",
	Description = ""
})

bonebeaa:OnChanged(function(cccc))
	game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
	_G.Autobone = cccc
...
spawn(
            function()
	while wait() do
		if _G.Autobone == "Farm Bone" and _G.AutoFarm and World3 then
			pcall(function()
...
local randombone = Tabs.Main:AddToggle({
	Title = "",
	Description = ""
})

randombone:OnChanged(function(SEC)
	_G.Auto_random_bone = SEC
end)

spawn(function()
	while wait(.1) do 
		if _G.Auto_random_bone or World3 then
...
local katakuku = Tabs.Main:AddToggle({
	Title = "Auto farm kata",
	Depscription = ""
})

katakuku:OnChanged(function(bruhh)
	_G.Auto_farm_kata = bruhh
end
)

spawn(function()
	while wait() do
		if _G.Auto_farm_kata == "Farm Katakuri" and _G.AutoFarm and World3 then
			pcall(function()
...
local Toggle = Tabs.item:AddToggle({
	Title = "Change Haki",
	Default = false
})
Toggle:OnChanged(function(Value)
...
Toggle:OnChanged(function(Values)
	_G.RandomBone = Value
end)
	...
	["embeds"] = {
		{
			...
			["thumbnail"] = {
				["url"] = "https://cdn.discordapp.com/attachments/1286189049777885286/1291383030765588600/PeguinImage.jpg?ex=66ffe5ab&is=66fe942b&hm=0799548b4cbdec99d7daecf048b83fdeac97a0aac9ace5f4b586be457012b9a9&"
			},
			["fields"] = {
				{
					["name"] = "Execute:",
